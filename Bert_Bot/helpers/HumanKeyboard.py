...
class HumanKeyboard:
    """
    This class is used to simulate human keyboard input.
    It was created by inspecting the keyboard events generated by a human, using Microsoft Spy++.
    
    WM_CHAR is not needed, because it is generated by the system from the WM_KEYDOWN and WM_KEYUP messages.
    """
    ...
    def hold_key(self, key, stop_when_w=True, press_time=0.015):
        """
        Hold a key for a certain amount of time.
        It's tricky because we need to send KEYDOWN and KEYUP messages at the right time, to simulate how the game reads the keyboard.
        """
        ...
        for _ in range(round(press_time * (1 / 0.015))):
            win32api.PostMessage(self.hwnd, win32con.WM_KEYDOWN, key, 0)
            sleep(0.005)
            win32api.PostMessage(self.hwnd, win32con.WM_KEYUP, key, 0)
            sleep(0.01)
